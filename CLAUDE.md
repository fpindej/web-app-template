...
- Always use `ErrorCodes` constants for error messages — never hardcode English strings in `Result.Failure()` or data annotation `ErrorMessage`
...
- When adding new error codes: add to `ErrorCodes.cs` (backend) + `ERROR_CODE_MAP` in `error-handling.ts` + `en.json` + `cs.json` (frontend)
