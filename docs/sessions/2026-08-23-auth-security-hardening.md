...
| `init.ps1` | Secret generation changed from 48-byte to 64-byte random buffers for both `Authentication__Jwt__Key` and the OAuth encryption key | M4: 48 base64 bytes could yield fewer than 64 characters after stripping `/+=`; 64 bytes makes an under-length key astronomically unlikely, and startup validation catches the residual case |
...
| `src/backend/MyProject.Infrastructure/Persistence/Extensions/ApplicationBuilderExtensions.cs` | Seed-user log lines (failure, success, and already-exists paths) mask the email via `PiiMasker.MaskEmail` | L3: PII removed from logs |
